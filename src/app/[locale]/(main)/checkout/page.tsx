...
import { CheckoutCartSummary } from '@/features/order/components/checkout-cart-summary';
    ...
    <main className="min-h-screen bg-[oklch(0.988_0.003_60)] text-stone-950 dark:bg-[oklch(0.115_0.01_285)] dark:text-white">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10">
        <header className="space-y-4 rounded-[2rem] border border-stone-200/80 bg-white/80 p-6 shadow-sm shadow-stone-950/5 dark:border-white/10 dark:bg-white/5 dark:shadow-black/20">
          <Link href="/cart" className="text-sm font-medium text-rose-500 transition hover:text-rose-600 dark:text-rose-300">
            {t('backToCart')}
          </Link>
          <div>
        ...
        {cart && items.length > 0 ? (
          <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
            <CheckoutForm
              ...
              labels={{
                ...
                errors: {
                  ...
                  emptyCart: t('errors.emptyCart'),
                  outOfStock: t('errors.outOfStock'),
            ...
            <CheckoutCartSummary
              cart={cart}
              locale={locale}
              labels={{ title: t('summary'), subtotal: t('subtotal'), total: t('total') }}
            />
          </div>
        ) : (
